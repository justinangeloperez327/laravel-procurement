...
import { Check, Pencil, Plus, RotateCcw, Send, ThumbsUp } from 'lucide-react';
...
type WorkflowAction = 'submitted' | 'recommended' | 'approved' | 'returned';
...
function transition(plan: AnnualProcurementPlan, action: WorkflowAction) {
    if (action === 'returned') {
        const remarks = window.prompt('Reason for returning this APP:');
    ...
    router.patch(`/planning/annual-procurement-plans/${plan.id}/${endpoint}`);
}
...
function WorkflowButton({
    ...
    const config = {
        submitted: { label: 'Submit to BAC', icon: Send },
        recommended: { label: 'Recommend', icon: ThumbsUp },
    ...
    return (
        <Button
            ...
            size="sm"
            onClick={() => transition(plan, action)}
        >
            <Icon />
            {config.label}
        </Button>
                ...
                <div className="mx-auto w-full max-w-7xl space-y-6">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <Heading
                            ...
                            description="Consolidate PPMP requirements, submit them to the BAC, and route recommended plans to the HoPE for approval."
                        />
